...
async function seedTopicLessons() {
    console.log('📝 Seeding topic lessons in MongoDB...');
    ...
    try {
        ...
        for (const lessonData of lessons) {
            ...
            if (existingLesson) {
                console.log(`ℹ️  Lesson "${lessonData.id}" already exists, skipping...`);
                skipped++;
                continue;
            }
        ...
        console.log(`\nYou can now use the Summary and Personalization features on all topic pages!\n`);
        process.exit(0);
    } catch (error) {
